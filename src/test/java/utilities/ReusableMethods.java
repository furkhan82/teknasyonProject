...
public class ReusableMethods {
    ...
    public static void longPressElement(AppiumDriver driver, WebElement element) {
        // Dokunmatik girdiyi tanımlar
        Actions actions = new Actions(driver);
        actions.clickAndHold(element)
                .pause(Duration.ofSeconds(3)) // Uzun basma süresi
                .release()
                .perform();
...
public static void openNotificationBar(){
    ...
    if (driver instanceof AndroidDriver) {
        ...
        System.out.println("Notification Bar opened");
    } else {
        System.out.println("Driver AndroidDriver değil.");
    }
}

    public static void swipeRight(String elementText) {
        // Sağ kaydırmak için bir UiScrollable ve UiSelector kullanıyoruz
        AndroidDriver driver = (AndroidDriver) getAppiumDriver();
        driver.findElement(MobileBy.AndroidUIAutomator(
                "new UiScrollable(new UiSelector().scrollable(true).instance(0))"
                        + ".setAsHorizontalList().scrollIntoView(new UiSelector().textMatches(\""
                        + elementText + "\").instance(0))"));

        // Sağ kaydırma işlemi için belirli bir koordinat seti kullanılabilir
        // Bu işlem doğrudan kaydırma hareketi değil, sadece kaydırılmış durumda olup olmadığını kontrol eder.
    }
    ...
    public static void scrollWithUiScrollable(String elementText) {
        //herhangibir elemeniti bulana kadar kaydir
        AndroidDriver driver = (AndroidDriver) getAppiumDriver();
        driver.findElement(MobileBy.AndroidUIAutomator(
    ...
    public static void VerifyElementText(String customTitle) {
        AndroidDriver driver = (AndroidDriver) getAppiumDriver();
        WebElement webElement = driver.findElement(MobileBy.AndroidUIAutomator(
                "new UiSelector().description(\"" + customTitle + "\")"));
